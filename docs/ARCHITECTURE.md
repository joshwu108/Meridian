...
| D22 | **CC-2 xDS wire contract frozen** (MER-70 / [ADR-0008](adr/0008-xds-wire-contract.md)): the transport half of CC-2 (kernel half = ADR-0004). `type_url`→Meridian mapping (CDS = compiled `wire.PolicyRule`, EDS = `wire.Identity`, LDS/RDS reserved for Phase-5 L7); resources are **Meridian-native protos** (`meridian.config.v1.PolicyRule`/`Identity`, frozen field numbers) in an `Any` over the xDS transport — superseding the D21 interim JSON-in-`BytesValue`. Kernel **commit order** (independent of xDS push order): identity adds → policy adds → policy removes → identity deletes, ACK only after commit, never transiently widening (CC-5). `internal/agent/datapath` stays the sole `wire`↔`bpf` translator (D17). | Freezes the agent-config contract A-3 (MER-72) compiles against; keeps Meridian L4 semantics single-sourced with `pkg/wire`/ADR-0004 rather than forcing the envoy cluster/endpoint model. |
