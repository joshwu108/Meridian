...
| D20 | **Agent SOCKMAP attach path** (MER-57): the production agent loads `sock_ops`/`sk_msg` via `bpfobj` (the sole `bpf/` opener — depguard `wire-bpf-bridge` forbids `cmd/` and `internal/agent/attach` from importing `bpf/`) and attaches them through two managers in `internal/agent/attach`: `CgroupSockOpsManager` (sock_ops → cgroup v2 via `link.AttachCgroup`, `BPF_CGROUP_SOCK_OPS`) and `SkMsgSockhashManager` (sk_msg → `sockhash` fd via `link.RawAttachProgram`, `BPF_SK_MSG_VERDICT`). Both are instance-idempotent (re-`EnsureAttached` is a no-op; `Detach` is a no-op when unattached). Gated behind `meridian-agent --cgroup`; absent the flag the SOCKMAP path is disabled (no change for TC-only deploys). These are SECONDARY map loads — the primary `LoadCounter`/`LoadTcIngress` owns the schema-sentinel reconcile, so the SOCKMAP loaders skip it. | Moves SOCKMAP attach out of test-only code into the agent, unblocking the MER-51 P2.2 gate; keeps the `bpf/` import boundary intact via bpfobj |
| D21 | **ADS server + interim xDS resource encoding** (MER-54/55/56, control plane): the control plane serves compiled state to agents over a single ADS stream (`internal/control/ads`), adding the `google.golang.org/grpc` + `github.com/envoyproxy/go-control-plane` dependency (per D11 — recorded by the phase that first imports it). Per-(stream, type_url) version/nonce handshake: an ACK (request nonce == last sent, no `error_detail`) advances the accepted version; a NACK (`error_detail` set) **holds last-known-good** (CC-5 fail-closed — a rejected/partial config never widens allows); a stale/unknown nonce is ignored. A `control.Store.Watch()` change recompiles and re-pushes subscribed types in make-before-break order (CDS→EDS, LDS→RDS); the MER-55 agent stub ACKs on decode and NACKs on a contract violation. The cross-boundary resource payload is **INTERIM**: a JSON-marshalled `[]wire.PolicyRule` packed in a `google.protobuf.BytesValue` Any on the Cluster channel only (other channels versioned-but-empty) — a private server↔stub contract for CP-1/CP-3, **NOT a frozen wire contract**. | Stands up the agent-config transport with CC-5 made explicit and bound to the `Store` seam (rather than adopting go-control-plane's SnapshotCache). The interim encoding is **superseded by the CC-2 compiled-policy wire-contract freeze (Phase 3)** — see ROADMAP §"Cross-cutting decisions" CC-2 and the ADR-0004 frozen L4 schema; do not treat the BytesValue/JSON payload as the frozen contract. |
| D22 | **CC-2 xDS wire contract frozen** (MER-70 / [ADR-0008](adr/0008-xds-wire-contract.md)): the transport half of CC-2 (kernel half = ADR-0004). `type_url`→Meridian mapping (CDS = compiled `wire.PolicyRule`, EDS = `wire.Identity`, LDS/RDS reserved for Phase-5 L7); resources are a **versioned JSON payload** (envelope `{schema_version,kind,spec}` mirroring `wire.PolicyRule`/`wire.Identity`, decoded with `DisallowUnknownFields` + integer-width validation) in a `BytesValue` `Any` over the xDS transport — superseding the D21 interim by *freezing + versioning* the JSON, not by adding codegen. (MER-77 revised this from protoc-generated messages: no `protoc` toolchain on host/Lima, and a protoc dep cuts against D11; reversible if protoc is later provisioned.) Kernel **commit order** (independent of xDS push order): identity adds → policy adds → policy removes → identity deletes, ACK only after commit, never transiently widening (CC-5). `internal/agent/datapath` stays the sole `wire`↔`bpf` translator (D17). | Freezes the agent-config contract A-3 (MER-72) compiles against; keeps Meridian L4 semantics single-sourced with `pkg/wire`/ADR-0004 rather than forcing the envoy cluster/endpoint model. |
...
**CC-2 boundary (ADS interim encoding):** the ADS server's cross-boundary xDS
resource encoding (D21 above) is **interim and CC-2-pending** — the JSON-in-
`BytesValue` payload is a private server↔stub contract, **superseded by the
Phase-3 compiled-policy wire-contract freeze (CC-2)**. The real frozen contract is
ADR-0004 (L4 maps) plus the forthcoming CC-2 xDS-metadata ADR; do not treat the
interim encoding as frozen. (Recorded as D21 / MER-67.)

**SOCKMAP redirect rationale (as-built, MER-52 / D18–D20):** the P2.2-BENCH benchmark measured **no intra-node latency win** for short connect+first-byte flows on kernel 5.15.0-179 — p50 within run-to-run noise (~+6%) and a consistent p99 **regression** (~+280%), with redirect confirmed engaged. The verdict-gated SOCKMAP redirect path is therefore justified by **correctness and mTLS-offload-readiness — a security-gated capability (CC-5 / ROADMAP Top-risk #2), not a proven performance fast path on this kernel.** Its runtime invariant (a denied flow is never redirected) is what closes the bypass risk; bulk-transfer correctness is proven by P2.2 (MER-51). Re-benchmarking on newer kernels is future work.
