...
| MER-59 | `docs/PHASE2_TICKETS.md` | **PAUSED — blocked on MER-68.** Phase-2 EXIT gate (joins {49✅,51✅,56✅,52✅} satisfied), but its certification AC ("0 skips across 9 gates, CI-confirmed") is **not reproducible** while `make check-gate-skips` flakes (gates green in isolation/canonical targets; harness leaks kernel state). Honest WIP docs exist uncommitted in the tree; finalize with a clean run + CI link once MER-68 lands. Must cite MER-68 by ID. |
| MER-68 | this file | **ACTIVE** — make `check-gate-skips` deterministic (reap bpffs/netns/cgroup state between privileged gates). Gate-integrity blocker of the MER-59 EXIT; gates themselves are green. |
